...
//import { EventFrameRemoved } from '@farcaster/frame-core';
...
/*
    try {
      setStatus("loading");
      const result = await sdk.actions.removeFrame() as EventFrameRemoved;

      if (result.event === "frame_removed") {
        if (result.notificationDetails) {
          const context = await sdk.context;

          await fetch("/api/notis/delete-token", {
            method: "POST",
            headers: {
      ...
      *
      ...
      console.log(error);
      setStatus("error");
      setErrorMessage("Failed to remove notification token");
    }*/
